...
---@class TIMSABA.Api
local api_timsaba = {defines = require("lib.defines")}
...
---@param belt_sprites TIMSABA.Api.Defines.BeltSprites Spritesheet to use for the frozen patch.
---@return data.RotatedSprite|nil
function api_timsaba.get_transport_belt_frozen_patch(belt_sprites)
	local frozen_sprite_filenames =
    {
		[1] = "__TIMSABA__/graphics/entity/standard/transport-belt-1-frozen.png",
		[2] = "__TIMSABA__/graphics/entity/standard/transport-belt-2-frozen.png",
		[3] = "__TIMSABA__/graphics/entity/standard/transport-belt-3-frozen.png",
	}
...
---@class TIMSABA.BeltSpritePreset
...
local presets =
{
    [api_timsaba.defines.belt_presets.turbo] =
    {
		belt_animation_set =
        {
			alternate = true,
			animation_set =
            {
				filename = "__TIMSABA__/graphics/entity/space-age/turbo-transport-belt/turbo-transport-belt.png",
				priority = extra_high,
				width = 128,
				height = 128,
				scale = 0.5,
		...
		technology_icon =
        {
			icon = "__TIMSABA__/graphics/technology/space-age/turbo-transport-belt.png",
			icon_size = 256,
		},
...
---@global
---@type table Неизвестно что
belt_reader_gfx = belt_reader_gfx
meld(presets[api_timsaba.defines.belt_presets.turbo].belt_animation_set, belt_reader_gfx)

---Gets a copy of the sprite presets for the specified `belt_preset`.
---@param belt_preset TIMSABA.Api.Defines.BeltPresets
---@return TIMSABA.BeltSpritePreset
function api_timsaba.get_preset(belt_preset)
...
---@class TIMSABA.LogisticsTechnologyIconInputs
...
---@param inputs TIMSABA.LogisticsTechnologyIconInputs
---@return data.IconData[]
function api_timsaba.get_transport_belt_technology_icon(inputs)
	---@type data.IconData[]
...
---@param inputs TIMSABA.TransportBeltIconInputs
---@return data.IconData
...
---@class TIMSABA.TransportBeltIconInputs
...
---@param inputs TIMSABA.TransportBeltIconInputs
---@return data.IconData[]
function api_timsaba.get_transport_belt_icon(inputs)
...
---@class TIMSABA.SpriteTintInputs
---Color to tint the base sprite (gears, rails)
---@field base_tint data.Color?
---
...
---@class TIMSABA.TransportBeltAnimationSetInputs: TIMSABA.SpriteTintInputs
---Spritesheet to use for the animation set; if omitted, defaults to a suitable spritesheet.
---@field belt_sprites TIMSABA.Api.Defines.BeltSprites?

---Returns a complete `TransportBeltAnimationSet` definition.
---@param inputs TIMSABA.TransportBeltAnimationSetInputs
---@return data.TransportBeltAnimationSetWithCorners
function api_timsaba.get_transport_belt_animation_set(inputs)
	local belt_sprites = inputs.belt_sprites or api_timsaba.defines.belt_sprites.standard

	---@class TIMSABA.ReturnBeltAnimationSetLayerInputs
	...
	---@param layer_inputs TIMSABA.ReturnBeltAnimationSetLayerInputs
	...
	local transport_belt_animation_set =
	{
		...
		frozen_patch = api_timsaba.get_transport_belt_frozen_patch(belt_sprites),
	}
...
local function create_or_update_remnants_core(transport_belt_name, animation)
	...
	if not remnants then
		remnants =
        {
			...
			localised_name = {"", (data.raw["transport-belt"][transport_belt_name].localised_name or { "entity-name." .. transport_belt_name }), " (remnant)"},
...
---@param inputs TIMSABA.SpriteTintInputs Table of parameters that configure the remnant sprites.
---@deprecated Method renamed, use `api_timsaba.create_or_update_remnants(name, inputs)` instead
function api_timsaba.create_remnant(name, inputs)
	api_timsaba.create_or_update_remnants(name, inputs)
end
...
---@param inputs TIMSABA.SpriteTintInputs Table of parameters that configure the remnant sprites.
function api_timsaba.create_or_update_remnants(transport_belt_name, inputs)
	---@class TIMSABA.ReturnRemnantLayerInputs
	---@field blend_mode? data.BlendMode Blending mode for the layer.
	---@field layer "base"|"mask"|"arrows" "base", "mask" or "arrows" (standard). Determines specific spritesheet used by the layer.
	---@field tint? data.Color Color to tint the layer.
	---@field tint_as_overlay? boolean When true, the color blending will use Overlay rules.
	...
	---@param layer_inputs TIMSABA.ReturnRemnantLayerInputs
	---@return data.RotatedAnimation
...
---@param inputs TIMSABA.PresetTransformBeltInputs
local function transform_belt_and_related_connectables_preset(entity, inputs)
	local preset = api_timsaba.get_preset(inputs.preset)
	...
	api_timsaba.apply_belt_animation_set_and_update_related_connectables(entity, preset.belt_animation_set,
    {
...
local function transform_belt_and_related_connectables_layered(entity, inputs)
    ...
    ({
		name = entity.name,
		type_name = entity.type,
		icon_data = api_timsaba.get_transport_belt_icon
        ({
	...
	if logistics_technology_inputs.name then
        ...
        ({
			...
			type_name = "technology",
			icon_data = api_timsaba.get_transport_belt_technology_icon
            ({
	...
	if not animation_set_inputs.belt_sprites then
		...
		if entity.speed < (30 / 480) then
			animation_set_inputs.belt_sprites = api_timsaba.defines.belt_sprites.standard
		elseif entity.speed < (60 / 480) then
			animation_set_inputs.belt_sprites = api_timsaba.defines.belt_sprites.fast
		else
			animation_set_inputs.belt_sprites = api_timsaba.defines.belt_sprites.turbo
		end
	end

	local animation_set = api_timsaba.get_transport_belt_animation_set(animation_set_inputs)
	...
	api_timsaba.create_or_update_remnants(entity.name, animation_set_inputs)
...
---@class TIMSABA.TransportBeltConnectableEntity
...
---@class TIMSABA.LogisticsTechnologyInputs:TIMSABA.LogisticsTechnologyIconInputs
...
---@class TIMSABA.PresetTransformBeltInputs
---@field preset TIMSABA.Api.Defines.BeltPresets
---The technology prototype name of a technology that unlocks the associated transport belt, and
---which should use a Logistics-style technology icon that has been colored like the transport belts.
---
...
---@field forced_connectable_belt_entities TIMSABA.TransportBeltConnectableEntity[]?

---@class TIMSABA.TransformBeltInputs
---The options that configured how the transport belt will be colored, and which spritesheet to use.
---@field belt_animation_set TIMSABA.TransportBeltAnimationSetInputs
---
...
---@field belt_icon TIMSABA.TransportBeltIconInputs?
---
...
---@field logistics_technology TIMSABA.LogisticsTechnologyInputs?
---
---A list of entities to forcibly set to the `belt_animation_set` of the reference belt. Use when
---the automatic resolution of reference belts does not resolve correctly.
---@field forced_connectable_belt_entities TIMSABA.TransportBeltConnectableEntity[]?
...
---@param inputs (TIMSABA.TransformBeltInputs|TIMSABA.PresetTransformBeltInputs)?
function api_timsaba.transform_belt_and_related_connectables(name, inputs)
	local entity = data_transport_belt[name]
	if not entity then
		return
	end

	-- Принудительно кастим inputs к типу с пресетом для проверки
	local preset_inputs = ---@type TIMSABA.PresetTransformBeltInputs?
	inputs

	if preset_inputs and preset_inputs.preset then
		transform_belt_and_related_connectables_preset(entity, preset_inputs)
	else
		-- Сюда попадет либо TransformBeltInputs, либо nil
		-- Кастим к TransformBeltInputs, так как layered-функция ждет его
		local layered_inputs = ---@type TIMSABA.TransformBeltInputs?
		inputs
		transform_belt_and_related_connectables_layered(entity, layered_inputs)
	end
end

---@alias TIMSABA.TransportBeltInputsMapping { [data.EntityID]: TIMSABA.TransformBeltInputs|TIMSABA.PresetTransformBeltInputs }
...
---@param transport_belt_inputs_map TIMSABA.TransportBeltInputsMapping
function api_timsaba.transform_belts_and_related_connectables(transport_belt_inputs_map)
	for name, inputs in pairs(transport_belt_inputs_map) do
		api_timsaba.transform_belt_and_related_connectables(name, inputs)
	end
end
